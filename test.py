...
def cal_mae(img_root,gt_dmap_root,model_param_path):
    ...
    print("model_param_path:"+model_param_path+" mae:"+str(sum_mae/len(test_dataloader)))
...
def estimate_density_map_no_gt(img_root,gt_dmap_root,model_param_path,index):
    ...
    model.load_state_dict(torch.load(model_param_path))
    model.to(device)
    test_dataloader  = create_test_extra_dataloader(cfg.dataset_root)             # dataloader
    model.eval()
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_dataloader)):
            image = data['image'].to(cfg.device)
...
if __name__=="__main__":
    ...
    gt_dmap_root='test'
    model_param_path='./checkpoints/95.pth'
    ...
    estimate_density_map_no_gt(img_root,gt_dmap_root,model_param_path,index=None)     
